    ...
    try:
        ...
        for conn in connections:
            ...
            if up_delta > 0 or down_delta > 0:
                ...
                inbound_tag = metadata.get("inboundName") or metadata.get("inboundTag") or metadata.get("inbound") or ""
                if inbound_tag.startswith("inbound-"):
                    try:
                        ib_id = int(inbound_tag.split("-")[1])
