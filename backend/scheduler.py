...
from backend.config import settings, XRAY_LOG_PATH, HYSTERIA_LOG_PATH
    ...
    if now_ts - last_backup >= interval_seconds:
        ...
        try:
            ...
            if not backup_enable and not telegram_attempted:
                # If we don't save locally and didn't attempt to send to TG (e.g. TG not configured),
                # delete the local file immediately to avoid disk leaks
                try:
...
def detect_and_block_port_scans():
    """
    Сканирует access.log Xray и hysteria.log Hysteria 2.
    Если за последние 10 секунд клиент совершил запросы к более чем 200 уникальным IP,
    автоматически блокирует его (enable = 0) и отправляет Telegram-оповещение.
    """
    import re
    import json
    import datetime
    ...
    from backend.models import ClientStats, Inbound
    from backend.utils import read_last_lines
    from backend.xray import remove_client_api, write_xray_config
    from backend.hysteria import kick_client_hysteria_api, restart_hysteria

    now_ts = time.time()
    cutoff_ts = now_ts - 10 # 10 seconds sliding window
    
    # Списки уникальных IP для каждого email
    ...
    # 3. Проверяем превышение лимита в 200 уникальных IP
    need_config_update = False
            ...
            if len(dst_ips) > 200:
                ...
                for c in clients:
                    ...
                    if inbound:
                        ...
                        if inbound.protocol == "hysteria2":
                            kick_client_hysteria_api(inbound.id, email)
                        else:
                            remove_client_api(inbound.id, email)
                            
                need_config_update = True
                asyncio_notify_admin(email, block_reason, bot_token, tg_admin_ids)
